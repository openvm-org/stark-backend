...
// Inline PTX: emit an explicit add.cc.u32 / addc.cc.u32 chain so ptxas lowers
// it to IADD3.X (native 32-bit add-with-carry) on sm_120 instead of falling
// back to ISETP+SEL+IADD.64 from the (t >> 32) idiom.
...
// limb's carry. This maps directly to Blackwell's IMAD.WIDE (32×32 + 64 → 64
// in one instruction).
// ---------------------------------------------------------------------------
