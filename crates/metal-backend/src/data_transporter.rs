...
) -> AirProvingContext<MetalBackend> {
    assert!(
        cpu_ctx.cached_mains.is_empty(),
        "CPU to GPU transfer of cached traces not supported"
    );
    let trace = transport_matrix_h2d_col_major(&cpu_ctx.common_main);
    AirProvingContext {
        cached_mains: vec![],
        common_main: trace,
...
pub fn transport_proving_ctx_to_host(
    gpu_ctx: ProvingContext<MetalBackend>,
    l_skip: usize,
...
pub fn transport_air_proving_ctx_to_host(
    gpu_ctx: AirProvingContext<MetalBackend>,
    l_skip: usize,
) -> AirProvingContext<CpuBackend<SC>> {
    let trace = transport_matrix_d2h_col_major(&gpu_ctx.common_main);
    let cached_mains = gpu_ctx
        ...
        .map(|mat| {
            ...
            CommittedTraceData {
                ...
                trace: transport_matrix_d2h_col_major(&mat.trace),
                data: Arc::new(StackedPcsData {
                    layout: mat.data.layout.clone(),
                    matrix: transport_matrix_d2h_col_major(&evals_matrix),
...
pub fn transport_merkle_tree_to_host(tree: &MerkleTreeMetal<F, Digest>) -> MerkleTree<F, Digest> {
    let backing_matrix =
        transport_matrix_d2h_col_major(tree.backing_matrix.as_ref().unwrap());
    let digest_layers = tree
        .digest_layers
        .iter()
