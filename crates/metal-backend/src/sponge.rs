...
impl Clone for DuplexSpongeMetal {
    fn clone(&self) -> Self {
        debug!("DuplexSpongeMetal::clone start");
        let mut new = Self {
        ...
        if !self.device.is_empty() {
            debug!("DuplexSpongeMetal::clone syncing");
            let _ = new.sync_h2d();
            debug!("DuplexSpongeMetal::clone synced");
        }
        debug!("DuplexSpongeMetal::clone done");
        new
    }
}
...
impl FiatShamirTranscript<SC> for DuplexSpongeMetal {
    ...
    #[inline]
    fn observe_commit(&mut self, digest: Digest) {
        debug!("DuplexSpongeMetal::observe_commit start");
        for x in digest {
            self.observe(x);
        }
        debug!("DuplexSpongeMetal::observe_commit done");
    }
}
