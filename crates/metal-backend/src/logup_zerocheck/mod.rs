...
use crate::{
    ...
    metal::logup_zerocheck::{fold_selectors_round0, interpolate_columns_gpu, MainMatrixPtrs},
    metal::sumcheck::batch_fold_mle,
    poly::EqEvalLayers,
    prelude::{EF, F, SC},
            ...
            let (log_output_heights, widths, mut output_mats): (Vec<_>, Vec<_>, Vec<_>) =
                input_mats
                    ...
                    .map(|mat| {
                        ...
                        let output_mat = MetalMatrix::<EF>::with_capacity(output_height, width);
