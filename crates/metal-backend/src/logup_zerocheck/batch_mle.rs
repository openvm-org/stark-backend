    ...
    pub fn new(
        ...
        for t in traces.iter() {
            ...
            zc_ctxs_h.push(ZerocheckCtx {
                ...
                num_y: t.num_y,
                d_eq_xi: t.eq_xi_ptr,
                d_rules: air_pk.other_data.zerocheck_mle.inner.d_rules.as_ptr() as *const _,
...
fn evaluate_mle_constraints_metal_batch(
    _block_ctxs: &MetalBuffer<BlockCtx>,
    _zc_ctxs: &MetalBuffer<ZerocheckCtx>,
    _air_block_offsets: &MetalBuffer<u32>,
    _lambda_pows: &MetalBuffer<EF>,
    _lambda_len: usize,
    num_x: u32,
    _threads_per_block: u32,
) -> MetalBuffer<EF> {
    let num_airs = _zc_ctxs.len();
    // TODO: dispatch zerocheck_batch_eval_mle Metal kernel when available.
    MetalBuffer::<EF>::with_capacity(num_airs * num_x as usize)
}

fn evaluate_mle_interactions_metal_batch(
    _block_ctxs: &MetalBuffer<BlockCtx>,
    _logup_ctxs: &MetalBuffer<LogupCtx>,
    _air_block_offsets: &MetalBuffer<u32>,
    num_x: u32,
    _threads_per_block: u32,
) -> MetalBuffer<Frac<EF>> {
    let num_airs = _logup_ctxs.len();
    // TODO: dispatch logup_batch_eval_mle Metal kernel when available.
    MetalBuffer::<Frac<EF>>::with_capacity(num_airs * num_x as usize)
}
