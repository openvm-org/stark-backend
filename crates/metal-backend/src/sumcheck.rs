    ...
    EF: ExtensionField<F>,
{
    ...
    let d_buffer_a = evals_ext.to_device();
    let d_buffer_b = MetalBuffer::<EF>::with_capacity(total_size / 2);

    // Set up pointer arrays on device
    let d_input_ptrs = MetalBuffer::<*const EF>::with_capacity(num_matrices);
    let d_output_ptrs = MetalBuffer::<*mut EF>::with_capacity(num_matrices);
    ...
    for round in 0..n {
        ...
        // Update pointer arrays (unified memory: direct write)
        let input_ptr = input_buf.as_ptr();
        let output_ptr = output_buf.as_mut_ptr();

        [input_ptr].copy_to(&d_input_ptrs).unwrap();
        [output_ptr].copy_to(&d_output_ptrs).unwrap();
...
pub fn sumcheck_prismalinear_metal(
    ...
    if domain_size == large_domain_size {
        ...
        let mut d_coeffs_large =
            MetalBuffer::<F>::with_capacity(num_x * width * large_domain_size);
    ...
    // Step 6: Read and convert to extension field (unified memory: direct read)
    ...
    let d_buffer_a = d_folded; // Reuse folded result
