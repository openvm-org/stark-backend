...
fn ensure_initialized(inverse: bool) {
    ...
    once.call_once(|| {
        let partial_twiddles = MetalBuffer::<[F; WINDOW_SIZE]>::with_capacity(WINDOW_NUM);
        let twiddles = MetalBuffer::<F>::with_capacity(32 + 64 + 128 + 256 + 512);
        unsafe {
