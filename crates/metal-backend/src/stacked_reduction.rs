use std::{array::from_fn, cmp::max, iter::zip, mem, sync::Arc};
...
pub struct StackedReductionMetal {
    ...
    q_width_max: u32,
    d_q_eval_ptrs: MetalBuffer<*const EF>,
    ...
    d_input_ptrs: MetalBuffer<*const EF>,
    d_output_ptrs: MetalBuffer<*mut EF>,
}
...
impl StackedReductionMetal {
    ...
    fn batch_sumcheck_poly_eval(
        ...
        q_eval_ptrs.copy_to(&self.d_q_eval_ptrs).unwrap();
    ...
    fn fold_mle_evals(&mut self, round: usize, u_round: EF) {
        ...
        let (folded_q_evals, input_ptrs, output_ptrs): (Vec<_>, Vec<_>, Vec<_>) = self
            ...
            .multiunzip();
        input_ptrs.copy_to(&self.d_input_ptrs).unwrap();
        output_ptrs.copy_to(&self.d_output_ptrs).unwrap();
    ...
    fn get_stacked_openings(&self) -> Vec<Vec<EF>> {
        self.q_evals.iter().map(|q| q.to_vec()).collect()
    }
}
