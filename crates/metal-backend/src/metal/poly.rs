...
pub unsafe fn eq_hypercube_stage_ext(out: *mut EF, x_i: EF, step: u32) -> Result<(), MetalError> {
    ...
    dispatch_sync(&pipeline, grid, group, |encoder| {
        encoder.set_bytes(0, std::mem::size_of::<EF>() as u64, &x_i as *const EF as *const c_void);
        encoder.set_bytes(1, 4, &step as *const u32 as *const c_void);
    })
}
...
pub unsafe fn mobius_eq_hypercube_stage_ext(
    ...
    let (grid, group) = grid_size_1d(step as usize, DEFAULT_THREADS_PER_GROUP);
    dispatch_sync(&pipeline, grid, group, |encoder| {
        encoder.set_bytes(0, std::mem::size_of::<EF>() as u64, &omega_i as *const EF as *const c_void);
        encoder.set_bytes(1, 4, &step as *const u32 as *const c_void);
    })
}
...
pub unsafe fn eval_poly_ext_at_point_from_base(
    ...
    let (grid, group) = grid_size_1d(coeff_len, DEFAULT_THREADS_PER_GROUP);
    dispatch_sync(&pipeline, grid, group, |encoder| {
