...
use super::{dispatch_sync, get_kernels, grid_size_1d, DEFAULT_THREADS_PER_GROUP, LOG_SIMD_SIZE};
...
pub unsafe fn mle_interpolate_stage_2d(
    ...
    let total = (padded_height as usize / 2) * width as usize;
    let (grid, group) = grid_size_1d(total, DEFAULT_THREADS_PER_GROUP);
    let is_e2c: u32 = if is_eval_to_coeff { 1 } else { 0 };
    dispatch_sync(&pipeline, grid, group, |encoder| {
        encoder.set_bytes(0, 2, &width as *const u16 as *const c_void);
        encoder.set_bytes(1, 4, &height as *const u32 as *const c_void);
        encoder.set_bytes(2, 4, &padded_height as *const u32 as *const c_void);
