...
pub struct InteractionEvalRules {
    ...
    /// ```
    pub(crate) d_pair_idxs: MetalBuffer<u32>,
...
pub struct EvalRules {
    /// Encoded rules
    pub d_rules: MetalBuffer<u128>,
    pub d_used_nodes: MetalBuffer<usize>,
    pub buffer_size: u32,
}

pub struct ZerocheckMonomials {
    pub d_headers: MetalBuffer<MonomialHeader>,
    pub d_variables: MetalBuffer<PackedVar>,
    pub d_lambda_terms: MetalBuffer<LambdaTerm<F>>,
    pub num_monomials: u32,
}

pub struct InteractionMonomials {
    pub d_numer_headers: MetalBuffer<MonomialHeader>,
    pub d_numer_variables: MetalBuffer<PackedVar>,
    pub d_numer_terms: MetalBuffer<InteractionMonomialTerm<F>>,
    pub num_numer_monomials: u32,
    pub d_denom_headers: MetalBuffer<MonomialHeader>,
    pub d_denom_variables: MetalBuffer<PackedVar>,
    pub d_denom_terms: MetalBuffer<InteractionMonomialTerm<F>>,
    pub num_denom_monomials: u32,
    pub max_fields_len: usize,
    pub num_interactions: u32,
}

fn to_device_or_dummy<T>(data: &[T]) -> MetalBuffer<T> {
    if data.is_empty() {
...
impl AirDataMetal {
    pub fn new(pk: &StarkProvingKey<SC>) -> Self {
...
impl ZerocheckMonomials {
    pub fn from_expanded(expanded: &ExpandedMonomials<F>) -> Self {
        // Validate bounds for all monomial headers to prevent out-of-bounds access in kernel
...
impl InteractionMonomials {
    pub fn from_expanded(expanded: &ExpandedInteractionMonomials<F>) -> Self {
        ...
        Self {
            d_numer_headers: to_device_or_dummy(&expanded.numer_headers),
            d_numer_variables: to_device_or_dummy(&expanded.numer_variables),
            d_numer_terms: to_device_or_dummy(&expanded.numer_terms),
            num_numer_monomials: expanded.numer_headers.len() as u32,
            d_denom_headers: to_device_or_dummy(&expanded.denom_headers),
            d_denom_variables: to_device_or_dummy(&expanded.denom_variables),
            d_denom_terms: to_device_or_dummy(&expanded.denom_terms),
...
impl InteractionEvalRules {
    pub fn new(symbolic_constraints: &SymbolicConstraints<F>) -> Self {
        ...
        if num_interactions == 0 {
            return Self {
                ...
                max_fields_len: 0,
                d_pair_idxs: MetalBuffer::with_capacity(1),
            };
        }
        ...
        let rules = SymbolicRulesMetal::new(&dag, false);
...
impl<const BUFFER_VARS: bool> ConstraintOnlyRules<BUFFER_VARS> {
    pub fn new(dag: &SymbolicExpressionDag<F>) -> Self {
        ...
        let rules = SymbolicRulesMetal::new(dag, BUFFER_VARS);
        // Build used_nodes with duplicates, preserving order from constraint_idx
        let used_nodes = dag
            .constraint_idx
            .iter()
...
impl EvalRules {
    pub fn dummy() -> Self {
        Self {
            d_rules: MetalBuffer::with_capacity(1),
            d_used_nodes: MetalBuffer::with_capacity(1),
            buffer_size: 0,
        }
    }
}
