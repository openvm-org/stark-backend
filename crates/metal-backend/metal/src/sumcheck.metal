...
kernel void sumcheck_mle_round_kernel(
    ...
    uint32_t grid_size = tpg; // single-dim grid
    for (uint32_t y = group_id * tpg + tid; y < half_height; y += grid_size) {
        for (uint32_t x_int = 1; x_int <= d; x_int++) {
            ...
            for (uint32_t mat_idx = 0; mat_idx < num_matrices; mat_idx++) {
                const device FpExt *input = reinterpret_cast<const device FpExt *>(input_matrices[mat_idx]);
...
kernel void fold_mle_kernel(
    ...
    const device FpExt *input = reinterpret_cast<const device FpExt *>(input_matrices[mat_idx]);
    device FpExt *output = reinterpret_cast<device FpExt *>(output_matrices[mat_idx]);
