...
kernel void zerocheck_mle_kernel(
    ...
    if (active_thread) {
        FpExt local_buffer[16]; // ZEROCHECK_BUFFER_THRESHOLD = 16
        thread FpExt *inter_buffer = local_buffer;
        uint32_t buffer_stride = 1;
...
kernel void logup_mle_kernel(
    ...
    if (active_thread) {
        FpExt local_buffer[10]; // LOGUP_BUFFER_THRESHOLD = 10
        thread FpExt *inter_buffer = local_buffer;
        uint32_t buffer_stride = 1;
