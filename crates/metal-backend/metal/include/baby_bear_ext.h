...
struct FpExt {
    ...
    // Multiply modulo x^4 - 11. Powers >= 4 wrap: x^4 = 11, so
    // we use NBETA = -11 (i.e., P-11) for the reduction terms.

    constexpr FpExt operator*(FpExt rhs) const thread {
        return FpExt(
            elems[0] * rhs.elems[0] + NBETA * (elems[1] * rhs.elems[3] + elems[2] * rhs.elems[2] + elems[3] * rhs.elems[1]),
            elems[0] * rhs.elems[1] + elems[1] * rhs.elems[0] + NBETA * (elems[2] * rhs.elems[3] + elems[3] * rhs.elems[2]),
            elems[0] * rhs.elems[2] + elems[1] * rhs.elems[1] + elems[2] * rhs.elems[0] + NBETA * (elems[3] * rhs.elems[3]),
...
constexpr inline FpExt inv(FpExt in) {
    Fp b0 = in.elems[0] * in.elems[0] + BETA * (in.elems[1] * (in.elems[3] + in.elems[3]) - in.elems[2] * in.elems[2]);
    Fp b2 = in.elems[0] * (in.elems[2] + in.elems[2]) - in.elems[1] * in.elems[1] + BETA * (in.elems[3] * in.elems[3]);
    ...
    return FpExt(
        ...
        -in.elems[1] * b0 + NBETA * in.elems[3] * b2,
