...
public:
    inline __device__ jacobian_t() {}
    inline __device__ jacobian_t(const field_t& x, const field_t& y, const field_t& z) :
                            X(x),             Y(y),             Z(z) {}
    ...
    inline __device__ jacobian_t& to_affine()
    {
        if(this->is_inf()) {
            this->identity();
            return *this;
            ...
            field_t Z_inv = Z.inverse();
            field_t Z2_inv = Z_inv * Z_inv;
            field_t Z3_inv = Z2_inv * Z_inv;
            X = X * Z2_inv;
            Y = Y * Z3_inv;
            Z = field_t::one();
            return *this;
    ...
    inline __device__ void identity()     { X.zero(); Y = field_t::one(); Z.zero(); }
