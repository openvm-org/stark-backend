...
fn verify_gkr_block_sumcheck<TS: FiatShamirTranscript>(
    ...
    for (block_idx, block_evals) in blocks.iter().enumerate() {
        ...
        // Derive s(0,...,0) from sumcheck constraint
        let boolean_sum = sum_at_boolean_points_except_origin(block_evals, k);
        let s_at_origin = claim - boolean_sum;

        // Interpolate at the sampled challenges
        let mut grid_evals = Vec::with_capacity(block_evals.len() + 1);
        grid_evals.push(s_at_origin);
        grid_evals.extend_from_slice(block_evals);
...
/// Sums evaluations over {0,1}^k excluding the origin.
///
/// `block_evals` contains values on the grid {0,1,2,3}^k **excluding** the all-zero point,
...
fn sum_at_boolean_points_except_origin(block_evals: &[EF], k: usize) -> EF {
    let mut sum = EF::ZERO;
...
mod tests {
    ...
    #[test]
    fn test_sum_at_boolean_points_except_origin() {
        let k = 3usize;
        ...
        for idx in 0..grid_len {
            ...
            grid[idx] = EF::from_canonical_u64(acc);
        }

        let block_evals = &grid[1..];
        ...
        let actual = sum_at_boolean_points_except_origin(block_evals, k);
