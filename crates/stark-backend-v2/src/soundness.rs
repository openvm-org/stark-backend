...
impl SoundnessAnalysis {
    ...
    /// WHIR soundness analysis.
    ///
    /// Error sources:
    ///
    /// 1. **Per sumcheck sub-round** (k sub-rounds per WHIR round):
    ///    - Proximity gaps: degree / |F_ext| (using degree after folding)
    ///    - Sumcheck: 2 / |F_ext|
    ///    - Compensated by folding_pow_bits
    ///
    /// 2. **Per non-final round**:
    ///    - Query error: ((1 + rate) / 2)^num_queries
    ///    - OOD error: degree / |F_ext|
    ///    - γ batching: (num_queries + 1) / |F_ext|
    ///    - Compensated by query_phase_pow_bits
    ///
    /// 3. **Final round** (coefficients sent directly):
    ///    - No OOD sample
    ///    - γ batching over queries only: num_queries / |F_ext|
    ///
    ///    Note: Unlike standard WHIR, our implementation performs γ batching in the final
    ///    round to simplify verifier logic. See `verify_whir()` for details.
    ///
    /// 4. **Initial μ batching**: num_stacked_columns / |F_ext|
    fn calculate_whir_soundness(
        ...
        assert!(
            num_stacked_columns >= 1,
            "WHIR requires at least 1 stacked column"
        );
        let mu_batching_bits = challenge_field_bits - (num_stacked_columns as f64).log2();
        ...
        for (round, round_config) in whir.rounds.iter().enumerate() {
            ...
            for _ in 0..k_whir {
                ...
                let sumcheck_bits = challenge_field_bits - 1.0 + whir.folding_pow_bits as f64;
                min_sumcheck_bits = min_sumcheck_bits.min(sumcheck_bits);
            }

            // Query error (all rounds)
            ...
            if is_final_round {
                // γ batching in final round: batches query claims only
                // Protected by query_phase_pow
                let gamma_batching_bits = challenge_field_bits
                    - (round_config.num_queries as f64).log2()
                    + whir.query_phase_pow_bits as f64;
                ...
                tracing::debug!(
                    ...
                    min_prox_gaps_bits, min_sumcheck_bits, gamma_batching_bits,
                );
            } else {
                // OOD sampling: z_0 sampled in commit phase
                let ood_bits = challenge_field_bits - current_log_degree as f64;
                min_ood_bits = min_ood_bits.min(ood_bits);

                // γ batching in non-final round: batches OOD + query claims
                // Protected by query_phase_pow
                let num_terms = round_config.num_queries + 1;
                let gamma_batching_bits = challenge_field_bits - (num_terms as f64).log2()
                    + whir.query_phase_pow_bits as f64;
                min_gamma_batching_bits = min_gamma_batching_bits.min(gamma_batching_bits);
                ...
                tracing::debug!(
                    ...
                    round, log_inv_rate, round_config.num_queries, query_bits,
                    min_prox_gaps_bits, min_sumcheck_bits, ood_bits, gamma_batching_bits,
                );
            }
    ...
    ///
    /// Bounds the probability that the folded polynomial is far from the RS code
    /// despite passing the proximity test.
    ///
    /// In unique decoding regime:
    /// Security bits = |F_ext| - log₂(degree)
    pub fn whir_proximity_gap_security(
        ...
        match soundness_type {
            SoundnessType::UniqueDecoding => challenge_field_bits - log_degree as f64,
        }
    }
}
