...
impl SoundnessCalculator {
    ...
    fn calculate_whir_soundness(
        ...
        for (round, round_config) in whir.rounds.iter().enumerate() {
            ...
            let batch_size = round_config.num_queries + round_config.num_odd_samples;
...
mod tests {
    ...
    // TODO: Change to 100 once we have mu batching PoW.
    const TARGET_SECURITY_BITS: usize = 83;
    ...
    fn test_logup_soundness() {
        let params = test_params();
        let security =
            SoundnessCalculator::calculate_logup_soundness(&params, challenge_field_bits());
