...
impl<EF: Field> GkrLogUpInstance<EF> {
        ...
        EF: ExtensionField<F>,
    {
        ...
        let mut bus_indices = vec![0; num_padded_interactions];
        ...
        for (i, interaction) in interactions.iter().enumerate() {
            ...
            .enumerate()
            {
                let evaluator = Evaluator::for_local_index(trace_view, local_index);

                let count = evaluator.eval_expr(&interaction.count);
                let count = match interaction.interaction_type {
                    InteractionType::Send => count,
                    InteractionType::Receive => -count,
                };
            ...
            bus_indices[i] = interaction.bus_index;
        }
    ...
    pub fn build_gkr_input_layer(&self, alpha_pows: &[EF]) -> Layer<EF> {
        debug_assert_eq!(self.counts.height(), self.sigmas.height());
        debug_assert_eq!(self.counts.width(), self.sigmas.width());
        debug_assert_eq!(self.counts.width(), self.bus_indices.len());

        let mut sum = EF::ZERO;
        let mut numerators = vec![];
        let mut denominators = vec![];
        for (col, bus_index) in self.bus_indices.iter().enumerate() {
            for row in 0..self.counts.height() {
                let numerator = self.counts.get(row, col);
                numerators.push(numerator);

                let denominator = alpha_pows[*bus_index] + self.sigmas.get(row, col);
