...
use std::{borrow::Borrow, cmp::max, fmt::Debug, iter::zip, marker::PhantomData};
...
use crate::{
    ...
    interaction::{
        ...
        HasInteractionChunkSize, InteractionBuilder, InteractionType, RapPhaseProverData,
...
impl<F, EF, Challenger> Default for GkrLogUpPhase<F, EF, Challenger> {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    Challenger: FieldChallenger<F>,
{
    ...
    fn partially_prove(
        ...
        let input_layers: Vec<_> = gkr_instances
            .iter()
            .map(|gkr_instance| gkr_instance.build_gkr_input_layer(&alpha_pows))
            .collect_vec();
    ...
    AB: InteractionBuilder + PermutationAirBuilderWithExposedValues,
{
    ...
    for interaction in all_interactions {
        let sign = if interaction.interaction_type == InteractionType::Send {
            AB::ExprEF::ONE
        } else {
            AB::ExprEF::NEG_ONE
        };
        s_next += sign * gamma_pows.next().unwrap() * AB::ExprEF::from(interaction.count.clone());
