    ...
    fn eval_var(&self, symbolic_var: SymbolicVariable<F>) -> F {
        ...
        match symbolic_var.entry {
            Entry::Preprocessed { offset } => {
                self.preprocessed.unwrap().get((n + offset) % height, index).expect("matrix index out of bounds")
            }
            Entry::Main { part_index, offset } => {
                self.partitioned_main[part_index].get((n + offset) % height, index).expect("matrix index out of bounds")
            }
