...
pub fn log_up_security_params_baby_bear_128_bits(
    ...
    // remaining gap. Floor at `MIN_BABY_BEAR_LOGUP_POW_BITS` to retain the reference 128-bit
    // configuration's baseline margin for unique-decoding configs. The authoritative check that the
    // resulting params clear the target is `test_all_production_configs` in the backend's
    // soundness tests.
