...
type ValMmcs<P> = MerkleTreeMmcs<KoalaBear, Bn254Fr, Hash<P>, Compress<P>, 1>;
...
pub type KoalaBearPermutationRootConfig<P> =
    StarkConfig<Pcs<P>, RapPhase<P>, Challenge, Challenger<P>>;
pub type KoalaBearPoseidon2RootConfig = KoalaBearPermutationRootConfig<Perm>;
pub type KoalaBearPoseidon2RootEngine = KoalaBearPermutationRootEngine<Perm>;

assert_sc_compatible_with_serde!(KoalaBearPoseidon2RootConfig);

pub struct KoalaBearPermutationRootEngine<P>
where
    P: CryptographicPermutation<[Bn254Fr; WIDTH]> + Clone,
{
    pub fri_params: FriParameters,
    pub config: KoalaBearPermutationRootConfig<P>,
    pub perm: P,
    pub max_constraint_degree: usize,
}

impl<P> StarkEngine<KoalaBearPermutationRootConfig<P>> for KoalaBearPermutationRootEngine<P>
where
    P: CryptographicPermutation<[Bn254Fr; WIDTH]> + Clone,
{
    fn config(&self) -> &KoalaBearPermutationRootConfig<P> {
        &self.config
    }
...
pub fn default_engine() -> KoalaBearPoseidon2RootEngine {
    default_engine_impl(SecurityParameters::standard_fast())
}

/// `pcs_log_degree` is the upper bound on the log_2(PCS polynomial degree).
fn default_engine_impl(security_params: SecurityParameters) -> KoalaBearPoseidon2RootEngine {
    let perm = root_perm();
    engine_from_perm(perm, security_params)
}

/// `pcs_log_degree` is the upper bound on the log_2(PCS polynomial degree).
pub fn default_config(perm: &Perm) -> KoalaBearPoseidon2RootConfig {
...
) -> KoalaBearPermutationRootEngine<P>
where
    P: CryptographicPermutation<[Bn254Fr; WIDTH]> + Clone,
{
    ...
    KoalaBearPermutationRootEngine {
        config,
        perm,
        fri_params,
...
pub fn config_from_perm<P>(
    perm: &P,
    security_params: SecurityParameters,
) -> KoalaBearPermutationRootConfig<P>
where
    P: CryptographicPermutation<[Bn254Fr; WIDTH]> + Clone,
{
    ...
    KoalaBearPermutationRootConfig::new(pcs, rap_phase)
}
...
impl StarkFriEngine<KoalaBearPoseidon2RootConfig> for KoalaBearPoseidon2RootEngine {
