...
In our use case (LogUp), the denominators have the form `q(x) = alpha + f(x)` where `alpha` is a random challenge. `log_gkr_input_evals` returns leaves **without** alpha applied, plus the `alpha` value. The `alpha` addition is then done inside `fractional_sumcheck_gpu` during tree build: the first layer uses `frac_build_tree_layer(..., apply_alpha=true)` to apply alpha while building parents, and `frac_add_alpha` applies alpha to the preserved second half needed for later reverts. The GKR prover itself is generic over any `(p, q)` leaf fractions.
...
**Buffer strategy:** The `layer` buffer (size `2^n`) holds the tree and is needed for reverts in non-last GKR rounds. In those rounds, data moves from `layer` to `work_buffer` on the first fused fold, then continues in-place on `work_buffer` to preserve `layer`. On the **last GKR round** (`j = n`), no future reverts are needed, so scheduling is flexible: the implementation may use ping-pong between `layer` and `work_buffer` when capacity permits (to use non-aliasing kernels), or stay in-place otherwise. The final fold after the last sumcheck round is standalone (no next round to compute) and runs on whichever buffer currently holds active data.
...
- A valid window width `w` exists (currently `w = GKR_WINDOW_SIZE = 3`)
- `rem_n >= min_n`, where `min_n` defaults to `GKR_WINDOW_DEFAULT_MIN_N = 22` and can be overridden with `SWIRL_CUDA_GKR_PRECOMPUTE_M_MIN_N`
- At least `w` sumcheck rounds remain (`rounds_left >= w`)
- Estimated tail parallelism meets threshold: `num_tail_blocks(rem_n, w, tail_tile) >= min_blocks`, where `min_blocks` defaults to 64 (`SWIRL_CUDA_GKR_PRECOMPUTE_M_MIN_BLOCKS`)

`tail_tile` is chosen adaptively from a target block count (default 1024, `SWIRL_CUDA_GKR_PRECOMPUTE_M_TARGET_BLOCKS`) and clamped to `[256, 4096]`, unless explicitly set by `SWIRL_CUDA_GKR_PRECOMPUTE_M_TAIL_TILE`.
