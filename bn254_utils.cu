...
template <typename ACC> static __device__ void bn254_mds_external_accessor(ACC &s, int WIDTH) {
    Bn254Fr sum = s.get_bn254(0);
    for (int i = 1; i < WIDTH; i++)
        sum = bn254_add(sum, s.get_bn254(i));
    for (int i = 0; i < WIDTH; i++)
        s.set_bn254(i, bn254_add(s.get_bn254(i), sum));
}
...
template <typename ACC> static __device__ void bn254_mds_internal_accessor(ACC &s, int WIDTH) {
    Bn254Fr sum = s.get_bn254(0);
    for (int i = 1; i < WIDTH; i++)
        sum = bn254_add(sum, s.get_bn254(i));
    for (int i = 0; i < WIDTH - 1; i++) {
        auto si = bn254_add(s.get_bn254(i), sum);
        s.set_bn254(i, si);
    }
    auto si = bn254_add(bn254_double(s.get_bn254(WIDTH - 1)), sum);
    s.set_bn254(WIDTH - 1, si);
}

// ---------------------------------------------------------------------------
// Poseidon2 permutation (width 3, rF=8, rP=56). Matches p3-bn254's
// Poseidon2Bn254<3> (HorizenLabs constants). Round constants must be uploaded
// to constant memory via cudaMemcpyToSymbol before launching a kernel that
// calls bn254_poseidon2_permute.
// ---------------------------------------------------------------------------
